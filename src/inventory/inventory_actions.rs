...
use crate::util::color::{DARK_RED, GOLD, LIGHT_BLUE, LIGHT_CYAN, LIGHT_GREEN, ORANGE, RED, Color, WHITE, YELLOW};
...
pub fn use_item(inventory_id: usize, tcod: &mut GameFramework, game: &mut GameEngine) {
    ...
    if let Some(item) = &game.entities[PLAYER].inventory[inventory_id].item {
        ...
        match on_use(inventory_id, tcod, game) {
            ...
            UseResult::Cancelled => {
                game.messages.add("Cancelled", Color::from(WHITE));
            }
        }
    } else {
        game.messages.add(format!("The {} cannot be used.", game.entities[PLAYER].inventory[inventory_id].name), Color::from(WHITE));
    }
}
...
pub fn drop_item(inventory_id: usize, _: &mut GameFramework, game: &mut GameEngine) {
    ...
    game.messages.add(format!("You dropped the {}.", item.name), Color::from(YELLOW));
...
pub fn cast_lightning(
    ...
    if let Some(monster_id) = monster_id {
        ...
        messages.add("No enemies are within range.", Color::from(RED));
        UseResult::Cancelled
    }
}
...
pub fn cast_confuse(_inventory_id: usize, tcod: &mut GameFramework, game: &mut GameEngine) -> UseResult {
    ...
    if let Some(monster_id) = monster_id {
        ...
        game.messages.add(format!("The eyes of the {} glaze over, and it starts to stumble around.", game.entities[monster_id].name), Color::from(LIGHT_GREEN));
        UseResult::UsedUp
    } else {
        game.messages.add("No enemy is close enough to strike", Color::from(RED));
        UseResult::Cancelled
    }
}

pub fn cast_fireball(_inventory_id: usize, tcod: &mut GameFramework, game: &mut GameEngine) -> UseResult {
    game.messages.add("Left-click a tile to cast a fireball at it; right-click or Esc to cancel", Color::from(LIGHT_CYAN));
    ...
    let event_bus = game.event_bus.borrow_mut();
    let messages = game.messages.borrow_mut();
    messages.add(format!("The fireball explodes, burning everything within {} tiles.", FIREBALL_RADIUS), Color::from(ORANGE));
    let mut xp_to_gain = 0;
    for (id, obj) in entities.iter_mut().enumerate() {
        if obj.distance(x, y) <= FIREBALL_RADIUS as f32 && obj.fighter.is_some() {
            game.messages.add(format!("The {} gets burned for {} hit points.", obj.name, FIREBALL_DAMAGE), Color::from(ORANGE));
        ...
        Some(item) => {
            match item {
                Item::Artifact {name, value} => {
                    game.messages.add(format!("This artifact is named {} and has a value of {}", name, value), Color::from(GOLD));
                    return UseResult::UsedAndKept
                },
                _ => {
                    game.messages.add("Error: examine_artifact was called with an item that was not an artifact", Color::from(DARK_RED));
                    return UseResult::Cancelled
                }
            };
        },
        None => {
            game.messages.add("Error: examine_artifact was called when there was no item", Color::from(DARK_RED));
            return UseResult::Cancelled
        }
    };
}
